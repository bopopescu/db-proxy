...
static chassis_options_t * network_mysqld_replicant_plugin_get_options(chassis_plugin_config *config) {
    if (config->opts == NULL) {
        ...
        chassis_options_add(opts, "replicant-username",                  0, 0, G_OPTION_ARG_STRING, &(config->mysqld_username), "username", "",
                            NULL, NULL, 0);
        chassis_options_add(opts, "replicant-password",                  0, 0, G_OPTION_ARG_STRING, &(config->mysqld_password), "password", "",
                            NULL, NULL, 0);
        chassis_options_add(opts, "replicant-read-binlogs",              0, 0, G_OPTION_ARG_FILENAME_ARRAY, &(config->read_binlogs), "binlog files", "",
                            NULL, NULL, 0);
