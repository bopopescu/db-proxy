    ...
    if (sql_type == 1) {
        ...
        if (token_id == TK_SQL_SET) {
            ...
            if (token_id == TK_OBRACE) {
				...
				g_log_dbproxy(g_message, "find column found %d,  start %d", found, start); 
                k = found - start + 1;
                single_insert_gap = k;
...
//    split_column = g_ptr_array_new();
    ...
    GArray* columns = get_column_index(tokens, table_name, dt->column_name, sql_type, table+1, split_column, dt->split_table_method);
    ...
    //3. ƴ��SQL
    GPtrArray* sqls = combine_sql(tokens, table, columns, dt->table_num, is_shadow, sql_type, dt->split_table_method, token_id, split_column);
    g_array_free(columns, TRUE);
