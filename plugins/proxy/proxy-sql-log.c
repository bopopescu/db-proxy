...
char* gen_json_data(	char* log_time, 
										char* client_ip, 
										char* client_db_name, 
										char* client_db_user, 
										char* server_ip, 
...
//2018-01-11 16:26:06.233889: C_begin:2018-01-11 16:26:06.232305 C:10.10.20.133:55779 C_db:flume C_usr:wheat S:10.99.1.151:3306(thread_id:374357) S_db:flume S_usr:wheat inj(type:4 bytes:0 rows:0) 1.077(ms) OK Query:SELECT * FROM `xes_flume_conf` LIMIT 0, 1000
  
	cJSON * js_body ;

	cJSON_AddItemToObject(root, "data",  js_body = cJSON_CreateObject());
	cJSON_AddStringToObject(js_body, "log_time", log_time);
	cJSON_AddStringToObject(js_body,"client_ip", client_ip);
	cJSON_AddStringToObject(js_body,"client_db_name", client_db_name);
	cJSON_AddStringToObject(js_body,"client_db_user", client_db_user);
	cJSON_AddStringToObject(js_body,"server_ip", server_ip);
	cJSON_AddNumberToObject(js_body,"thread_id", thread_id);
	cJSON_AddStringToObject(js_body,"server_db_name", server_db_name);
	cJSON_AddStringToObject(js_body,"server_db_user", server_db_user);
	cJSON_AddNumberToObject(js_body,"inj_type", inj_type);
	cJSON_AddNumberToObject(js_body,"inj_bytes", inj_bytes);
	cJSON_AddNumberToObject(js_body,"rows", rows);
	cJSON_AddStringToObject(js_body,"duration", duration);
	cJSON_AddStringToObject(js_body,"code", code );
	cJSON_AddStringToObject(js_body,"sql", sql);
...
log_sql_backend(sql_log_t *sql_log, network_mysqld_con *con, injection *inj)
{
	...
	*/
	...
	char duration[30]; //定义一个字符数组，来存储数据
	sprintf(duration,"%f", latency_ms ); //格式串同printf（）格式要求

	char* json_msg = gen_json_data(begin_time->str, 
															NETWORK_SOCKET_SRC_NAME(con->client), 
															NETWORK_SOCKET_DB_NAME(con->client), 
															NETWORK_SOCKET_USR_NAME(con->client),
															...
															inj->rows,
															duration, 
															inj->qstat.query_status == MYSQLD_PACKET_OK ? "OK" : "ERR",
															(inj->query->len > 1 ? (inj->query->str + 1) : ""));
