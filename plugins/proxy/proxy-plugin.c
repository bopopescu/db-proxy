...
GPtrArray* combine_sql(GPtrArray* tokens, gint table, GArray* columns, guint num, gboolean has_suffix, gint sql_type, gint split_table_method, gint token_id,  GArray* split_column) {
	g_log_dbproxy(g_message, "处理SQL, 分表类型: %d, 表名位置: %d, 分表字段对应值的个数:%d", split_table_method, table, columns->len); 
        ...
        */
        ...
        if(3 == sql_type) {
            ...
            for (i = 0; i < split_column->len; ++i) {
                ...
                g_log_dbproxy(g_message, "批量插入,提取数据: %s", tmp->str); 
            ...
            g_log_dbproxy(g_message, "批量插入,字段为: %s", columns_str->str); 
            ...
            */
            ...
            if (  1 == split_table_method ) {
                ...
                for (i = 0; i < clen; ++i) {
                    ...
                    g_log_dbproxy(g_message, "分表字段的值hash:  %lu 取余 %d", ul, num ); 
            ...
            for (m = 0; m < num; ++m) {
                if (mt[m]->len > 0) {
                    ...
                    g_log_dbproxy(g_message,"in查询sql: %s", sql->str);
                }
