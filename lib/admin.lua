...
function read_query(packet)
    ...
    elseif string.find(query:lower(), "^%s*show%s+tables%s*") then
        ...
        fields = {
            ...
            { 
                name = "分表类型 1:int, 2:string, 3:年, 4:年-月, 5:年-月-日",
                type = proxy.MYSQL_TYPE_STRING
            }
        }
        ...
        if nil ~= tables then
            ...
            for _,k in pairs(key_table) do
                column = string_split(k, ".")
                split_type = table.remove(column)
                table_numbers =  table.remove(column)
                split_column =  table.remove(column)
                table_name =  table.remove(column)
                db_name =  table.remove(column)
                rows[#rows + 1] = {db_name, table_name, split_column, table_numbers, split_type}
            end
        end
