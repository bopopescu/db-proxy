...
network_socket *self_connect(network_mysqld_con *con, network_backend_t *backend, GHashTable *pwd_table, gint is_oligarch) {
	...
	g_log_dbproxy(g_message, "self_connect  connect real mysql ip is  %s", sock->dst->name->str ); 
	...
	*/
	...
	/*
    zhangming 2018/1/14 17:12
	off_t to_write = 58 + con->client->response->username->len+1;
	当dbproxy向real mysql 转发时,使用real mysql 的账号
	*/
	off_t to_write = 58 + user_len;
   ...
   /*
   zhangming 2018/1/14 16:12
   这里使用real mysql 账号
   */
	g_string_append_len(data, user, user_len);
	
	 g_log_dbproxy(g_message, "self_connect  con->client->response->username->str is  %s, on->client->response->username->len is %d", con->client->response->username->str, con->client->response->username->len );
