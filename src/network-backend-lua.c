...
again:
    ...
    if (index < bs->backends->len)
        b = bs->backends->pdata[index];

    if (!b) {
        g_rw_lock_reader_unlock(&bs->backends_lock);
        g_log_dbproxy(g_warning, "%s backend failed, backend %d is not exist",
                            (state_type == BACKEND_STATE_REMOVING) ? "remove" :
                                (state_type == BACKEND_STATE_OFFLINE ? "offline" : "online" ),
                                index);
        return 1;
    }

    state = g_atomic_int_get(&b->state);
    ...
    if (state_type == BACKEND_STATE_REMOVING ||
                state_type == BACKEND_STATE_OFFLINE) {
        ...
        if (state_type == BACKEND_STATE_OFFLINE) {
            g_rw_lock_writer_lock(&bs->backends_lock);
            if (state != g_atomic_int_get(&b->state)) {
        ...
        if (b->type == BACKEND_TYPE_RW && state_type == BACKEND_STATE_UNKNOWN) {
            SET_BACKEND_STATE(b, BACKEND_STATE_UP);
        } else {
